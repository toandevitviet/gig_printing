...
class ModelCatalogProduct extends Model {
	...
	//Get gifts  and list products

	public function getGift($slider_image_id) {

		$query = $this->db->query("SELECT  *  FROM " . DB_PREFIX ."slider_image_description INNER JOIN " . DB_PREFIX . "slider_image    ON " . DB_PREFIX ."slider_image_description.slider_image_id = ". DB_PREFIX . "slider_image.slider_image_id "); //WHERE sd.language_id = '1' 
		//echo '<pre>'; var_dump($query); die;
		if ($query->num_rows) {
			return array(
				'slider_image_id'         => $query->row['slider_image_id'],
	...
	public function getGifts($data = array()) {
		$sql ="SELECT  *  FROM " . DB_PREFIX ."cardgift INNER JOIN " . DB_PREFIX . "cardgift_description    ON " . DB_PREFIX ."cardgift.cardgift_id = ". DB_PREFIX . "cardgift_description.cardgift_id WHERE " . DB_PREFIX ."cardgift.status = '1' ";
		$query = $this->db->query($sql);
		return $query->rows;
	}
	...
	public function getRandomProduct() {
	   ...
	   $query = $this->db->query("SELECT DISTINCT * FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) WHERE p.product_id >= '" . (int)$randomrow . "' AND p.date_available <= NOW() AND p.status = '1' GROUP BY p.product_id order by p.product_id ASC");
	   //echo "SELECT DISTINCT * FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id)  WHERE p.product_id >= '" . (int)$randomrow . "' AND p.date_available <= NOW() AND p.status = '1' GROUP BY p.product_id order by p.product_id ASC";
	   //var_dump($query);die;
		return $query->rows;
	}

	// Retrieve a random product limit 3 to futured and home page
	public function getRandomProduct_limit3() {
	   $maxminid = $this->db->query("SELECT MAX(product_id) as max_id, MIN(product_id) as min_id from ". DB_PREFIX . "product p");
	   $maxrow = -1;
	   $minrow = -1;
	   if ($maxminid->num_rows) {
	   ...
	   $query = $this->db->query("SELECT DISTINCT *, pc.price AS spical_value  FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id)   LEFT JOIN " . DB_PREFIX . "product_special pc ON (p.product_id = pc.product_id) WHERE p.product_id >= '" . (int)$randomrow . "' AND p.date_available <= NOW() AND p.status = '1' GROUP BY p.product_id order by p.product_id ASC LIMIT 3");
		//echo "SELECT DISTINCT * FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id)   LEFT JOIN " . DB_PREFIX . "product_special pc ON (p.product_id = pc.product_id) WHERE p.product_id >= '" . (int)$randomrow . "' AND p.date_available <= NOW() AND p.status = '1' GROUP BY p.product_id order by p.product_id ASC LIMIT 3";
