...
<?php echo $column_left; ?>
  ...
  <?php if ($attention) { ?>
  <div class="attention"><?php echo $attention; ?><img src="catalog/view/theme/theme483/image/remove-small.png" alt="" class="close" /></div>
  <?php } ?>
  <?php if ($success) { ?>
  <div class="success"><?php echo $success; ?><img src="catalog/view/theme/theme483/image/remove-small.png" alt="" class="close" /></div>
  <?php } ?>
  <?php if ($error_warning) { ?>
  <div class="warning"><?php echo $error_warning; ?><img src="catalog/view/theme/theme483/image/remove-small.png" alt="" class="close" /></div>
  <?php } ?>
		...
		<tbody>
		  <?php foreach ($products as $product) { ?>
			<?php if(!empty($product['recurring'])): ?>
			  <tr>
				  <td colspan="6" style="border:none;"><image src="catalog/view/theme/theme483/image/reorder.png" alt="" title="" style="float:left;" /><span style="float:left;line-height:18px; margin-left:10px;"> 
		  ...
		  <tr>
			...
			<td class="name"><a href="<?php echo $product['href']; ?>"><?php echo $product['name']; ?></a>
			  ...
			  <div>
				...
				- <small class="custom_fff"><?php echo $option['name']; ?>: <?php echo $option['value']; ?></small><br />
				<?php } ?>
				<?php if (!empty($product['recurring'])): ?>
				- <small class="custom_fff"><?php echo $text_payment_profile ?>: <?php echo $product['profile_name'] ?></small>
				<?php endif; ?>
			  </div>
			...
			<td class="price"><?php echo $product['quantity']; ?>  </td>
  ...
  <div class="content shop-cart-content">
	...
	<table class="radio table table-bordered table-hover">
	  ...
	  <tr class="highlight">
			<td><div class="radio"><label><input type="radio" name="next" value="coupon" id="use_coupon" /><?php echo $text_use_coupon; ?></label></div></td>
	  </tr>
	  <?php } ?>
	  <?php if ($voucher_status) { ?>
	  <tr class="highlight">
		 <td><div class="radio"><label  ><input type="radio" name="next" value="voucher" id="use_voucher" /><?php echo $text_use_voucher; ?></label></div></td>
	  </tr>
	  <?php } ?>
	  <?php if ($reward_status) { ?>
	  <tr class="highlight">
		
		<td><div class="radio"><label  ><input type="radio" name="next" value="reward" id="use_reward" /><?php echo $text_use_reward; ?></label></div></td>
	  </tr>
	  <?php } ?>
  ...
  <div class="cart-module">
	<div id="coupon" class="content">
		<form class="form-horizontal" action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="coup">
			<div class="form-group">
				<label class="control-label col-sm-5" ><?php echo $entry_coupon; ?>&nbsp;</label>
	...
	<div id="voucher" class="content">
		<form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="gift" class="form-horizontal">
			<div class="form-group">
				<label class="control-label col-sm-5" ><?php echo $entry_voucher; ?>&nbsp;</label>
				<div class="controls col-sm-7">
	...
	<div id="reward" class="content">
		<form class="form-horizontal"  action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="rew">
			<div class="form-group">
				<label class="control-label col-sm-5" ><?php echo $entry_reward; ?>&nbsp;</label>
				<div class="controls col-sm-7">
...
$('#button-quote').live('click', function() {
	$.ajax({
		...
		beforeSend: function() {
			...
			$('#button-quote').after('<span class="wait">&nbsp;<img src="catalog/view/theme/theme483/image/loading.gif" alt="" /></span>');
		},
		complete: function() {
		...
		success: function(json) {
			...
			if (json['error']) {
				if (json['error']['warning']) {
					$('#notification').html('<div class="warning" style="display: none;">' + json['error']['warning'] + '<img src="catalog/view/theme/theme483/image/close-1.png" alt="" class="close" /></div>');
...
$('select[name=\'country_id\']').bind('change', function() {
	$.ajax({
		...
		dataType: 'json',
		beforeSend: function() {
			$('select[name=\'country_id\']').after('<span class="wait">&nbsp;<img src="catalog/view/theme/theme483/image/loading.gif" alt="" /></span>');
		},
		complete: function() {
			$('.wait').remove();
		},			
