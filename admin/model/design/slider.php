...
class ModelDesignslider extends Model {
	public function addslider($data) {
		...
		if (isset($data['slider_image'])) {
			foreach ($data['slider_image'] as $slider_image) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "slider_image SET slider_id = '" . (int)$slider_id . "', link = '" .  $this->db->escape($slider_image['link']) . "', image = '" .  $this->db->escape($slider_image['image']) . "', sorting='" . (int)$slider_image['sorting'] . "'");
				
				$slider_image_id = $this->db->getLastId();
				
				foreach ($slider_image['slider_image_description'] as $language_id => $slider_image_description) {				
					$this->db->query("INSERT INTO " . DB_PREFIX . "slider_image_description SET slider_image_id = '" . (int)$slider_image_id . "', language_id = '" . (int)$language_id . "', slider_id = '" . (int)$slider_id . "', title = '" .  $this->db->escape($slider_image_description['title']) . "', description = '" .  $this->db->escape($slider_image_description['description']) . "', price = '" .  $this->db->escape($slider_image_description['price']) . "', gift = '" .  $this->db->escape($slider_image_description['gift']) . "'");
				}
			}
		}		
	}
	...
	public function editslider($slider_id, $data) {
		...
		if (isset($data['slider_image'])) { 
			foreach ($data['slider_image'] as $slider_image) {	
				$this->db->query("UPDATE " . DB_PREFIX . "slider_image_description SET title = '" .  $this->db->escape($slider_image['slider_image_description'][1]['title']) . "', description = '" .  $this->db->escape($slider_image['slider_image_description'][1]['description']) . "', price = '" .  $this->db->escape($slider_image['slider_image_description'][1]['price']) . "',  gift = '" .  $this->db->escape($slider_image['slider_image_description'][1]['gift']) . "' WHERE slider_image_id = '" .  $slider_image['sorting'] . "' ");
				$this->db->query("UPDATE " . DB_PREFIX . "slider_image SET image = '" .  $slider_image['image']  . "', link = '" .  $slider_image['link'] . "' WHERE slider_image_id = '" .  $slider_image['sorting'] . "' ");
			}
		}			
	}
		...
		foreach ($slider_image_query->rows as $slider_image) {
			...
			foreach ($slider_image_description_query->rows as $slider_image_description) {			
				$slider_image_description_data[$slider_image_description['language_id']] = array('title' => $slider_image_description['title'],
																								...
																								'price'=>$slider_image_description['price'],
																								'gift'=>$slider_image_description['gift']
																								);
			}
	...
	public function getTotalsliders() {
		...
		// $this->db->query("INSERT INTO " . DB_PREFIX . "slider_image_description VALUES(64, 1, 1, 'resim4', 'alt başlık 4', 'açıklama 4', 111, 2);");
		// $this->db->query("INSERT INTO " . DB_PREFIX . "slider_image_description VALUES(63, 1, 1, 'resim 3', 'alt başlık 3', 'açıklama 3', 222, 3);");
		// $this->db->query("INSERT INTO " . DB_PREFIX . "slider_image_description VALUES(62, 1, 1, 'resim 2', 'alt başlık 2', 'açıklama 2', 333, 4);");
		// $this->db->query("INSERT INTO " . DB_PREFIX . "slider_image_description VALUES(61, 1, 1, 'resim 1', 'alt başlık 1', 'açıklama 1', 444, 5);");
