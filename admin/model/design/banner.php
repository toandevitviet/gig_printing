...
class ModelDesignBanner extends Model {
	...
	public function editBanner($banner_id, $data) {
		...
		if (isset($data['banner_image'])) { 
			foreach ($data['banner_image'] as $slider_image) {				
			$this->db->query("UPDATE " . DB_PREFIX . "banner_image_description SET title = '" .  $this->db->escape($slider_image['banner_image_description'][1]['title']) . "', description = '" .  $this->db->escape($slider_image['description']) . "'  WHERE banner_image_id = '" .  $slider_image['banner_image_id'] . "' ");
            $this->db->query("UPDATE " . DB_PREFIX . "banner_image SET image = '" .  $slider_image['image']  . "', link = '" .  $slider_image['link'] . "' WHERE banner_image_id = '" .  $slider_image['banner_image_id'] . "' ");
			}
		} 
	...
	public function getBannerImages($banner_id) {
		...
		foreach ($banner_image_query->rows as $banner_image) {
			...
			foreach ($banner_image_description_query->rows as $banner_image_description) {			
				$banner_image_description_data[$banner_image_description['language_id']] = array('title' => $banner_image_description['title'], 'banner_image_id' => $banner_image_description['banner_image_id'], 'description' => $banner_image_description['description']);
			}
